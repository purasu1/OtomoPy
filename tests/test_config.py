...
def _ids(relay_channels: dict[str, list[int]]) -> dict[str, set[int]]:
...
class TestRelayChannels:
    def test_add_returns_true_then_false(self, config: GuildConfig) -> None:
    ...
    def test_remove_returns_false_when_absent(self, config: GuildConfig) -> None:
    ...
    def test_remove_prunes_youtube_key_when_last_target_goes(self, config: GuildConfig) -> None:
    ...
    def test_remove_keeps_youtube_key_while_targets_remain(self, config: GuildConfig) -> None:
        config.add_relay_channel(GUILD_A, CHANNEL_1, YT_A)
        config.add_relay_channel(GUILD_A, CHANNEL_2, YT_A)
    ...
    def test_guilds_are_isolated(self, config: GuildConfig) -> None:
        config.add_relay_channel(GUILD_A, CHANNEL_1, YT_A)
    ...
    def test_get_all_youtube_channels_unions_across_guilds(self, config: GuildConfig) -> None:
        config.add_relay_channel(GUILD_A, CHANNEL_1, YT_A)
...
class TestBlacklist:
    ...
    def test_roundtrip_across_reopen(self, config: GuildConfig, reopen: Reopen) -> None:
    ...
    def test_remove_returns_false_for_other_guilds_entry(self, config: GuildConfig) -> None:
    ...
    def test_remove_roundtrip(self, config: GuildConfig, reopen: Reopen) -> None:
        config.add_blacklisted_user(GUILD_A, "Translator")
    ...
    def test_blacklist_is_per_guild(self, config: GuildConfig) -> None:
        config.add_blacklisted_user(GUILD_A, "Translator")
        assert config.is_user_blacklisted(GUILD_B, "Translator") is False

    def test_get_blacklisted_users(self, config: GuildConfig) -> None:
...
class TestEmotes:
    def test_set_get_roundtrip(self, config: GuildConfig, reopen: Reopen) -> None:
        config.set_emote("Hololive", "<:holo:123>")
        assert reopen().get_emote("hololive") == "<:holo:123>"

    def test_lookup_is_case_insensitive(self, config: GuildConfig) -> None:
        config.set_emote("DeepL", "<:deepl:1>")
        assert config.get_emote("deepl") == "<:deepl:1>"
        assert config.get_emote("DEEPL") == "<:deepl:1>"

    def test_get_missing_returns_default(self, config: GuildConfig) -> None:
        assert config.get_emote("nope") is None
        assert config.get_emote("nope", "fallback") == "fallback"

    def test_unset_missing_returns_false(self, config: GuildConfig) -> None:
        assert config.unset_emote("nope") is False

    def test_unset_roundtrip(self, config: GuildConfig, reopen: Reopen) -> None:
        config.set_emote("hololive", "<:holo:123>")
